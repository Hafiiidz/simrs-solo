...
<div class="modal-body">
    <div class="card card-bordered">
        <div class="card-body">
            <form action="{{ route('farmasi.edit-obat-farmasi-racikan') }}" method="post" id='frmEditObat'>
                @csrf
                <div class="mb-10">
                    <input type="hidden" name='resep_asal' value="{{ $resep->id }}">
                    <input type="hidden" name='obat_asal' value="{{ $obat->id }}">
                    <label for="nama_obat" class="form-label
                    required">Nama Obat</label>
                    <select name="id_data_obat" class="form-select" required data-placeholder="Select an option"
                        data-control="select2" id="id_data_obat">
                        <option value=""></option>
                        @foreach ($list_obat as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $obat->idobat ? 'selected' : '' }}>
